...
test('Linter: unused components', async assert => {
  ...
  let source = '<div></div>'
  assert.deepEqual(await linter.lint(source), [])

  source = '<import button from="./foo.html">'
  assert.deepEqual(await linter.lint(source), [{ type: 'UNUSED_COMPONENT', message: 'button component is unused' }])

  source = '<import button from="./foo.html"><import input from="./input.html"><button></button>'
  assert.deepEqual(await linter.lint(source), [{ type: 'UNUSED_COMPONENT', message: 'input component is unused' }])

  source = '<import button from="./foo.html"><import input from="./input.html">'
  assert.deepEqual(
    await linter.lint(source),
    [
