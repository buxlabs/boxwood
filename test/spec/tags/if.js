...
test('if', async assert => {
  var { template } = await compile('<if foo>bar</if>')
  assert.deepEqual(template({ foo: false }, escape), '')

  var { template } = await compile('<if foo>bar</if>')
  assert.deepEqual(template({ foo: true }, escape), 'bar')

  var { template } = await compile('<if foo>bar</if><if baz>qux</if>')
  ...
  var { template } = await compile('<if foo>bar</if><elseif baz>qux</elseif><else>quux</else>')
  assert.deepEqual(template({ foo: true, baz: true }, escape), 'bar')
  ...
  var { template } = await compile('<if foo>bar</if><elseif baz>qux</elseif><elseif quux>corge</elseif>')
  ...
  var { template } = await compile('<if foo and bar>baz</if>')
  ...
  var { template } = await compile('<if foo xor bar>baz</if>')
  assert.deepEqual(template({ foo: false, bar: true }, escape), 'baz')
  ...
  var { template } = await compile('<if foo nor bar>baz</if>')
  assert.deepEqual(template({ foo: false, bar: true }, escape), '')

  var { template } = await compile('<if foo nor bar>baz</if>')
  assert.deepEqual(template({ foo: false, bar: false }, escape), 'baz')

  var { template } = await compile('<if foo eq bar>baz</if>')
  ...
  var { template } = await compile('<if foo neq bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 42 }, escape), '')
  ...
  var { template } = await compile('<if foo neq="{42}">baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 42 }, escape), '')
  ...
  var { template } = await compile('<if foo is equal to="bar">baz</if>')
  assert.deepEqual(template({ foo: 'bar', bar: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo is equal to="{42}">baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 42 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not equal to="bar">baz</if>')
  assert.deepEqual(template({ foo: 'bar', bar: 'bar' }, escape), '')

  var { template } = await compile('<if foo is not equal to="{42}">baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 42 }, escape), '')
  ...
  var { template } = await compile('<if foo gt bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 30 }, escape), 'baz')

  var { template } = await compile('<if foo gt two>baz</if>')
  ...
  var { template } = await compile('<if foo gte bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 30 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo lt bar>baz</if>')
  ...
  var { template } = await compile('<if foo lte bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 30 }, escape), '')
  ...
  var { template } = await compile('<if foo lte="{bar.baz - 2}">baz</if>')
  assert.deepEqual(template({ foo: 2, bar: { baz: 10 } }, escape), 'baz')

  var { template } = await compile('<if foo lte="{bar.baz - 2}">baz</if>')
  assert.deepEqual(template({ foo: 100, bar: { baz: 10 } }, escape), '')

  var { template } = await compile('<if foo equals bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 42 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is less than bar>baz</if>')
  ...
  var { template } = await compile('<if foo is less than or equals bar>baz</if>')
  assert.deepEqual(template({ foo: 30, bar: 40 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is greater than bar>baz</if>')
  ...
  var { template } = await compile('<if foo is greater than or equals bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not greater than bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not greater than five>baz</if>')
  ...
  var { template } = await compile('<if foo is present>bar</if>')
  ...
  var { template } = await compile('<if foo.bar is not present>baz</if>')
  assert.deepEqual(template({ foo: '' }, escape), 'baz')

  var { template } = await compile('<if foo.bar is not present>baz</if>')
  assert.deepEqual(template({ foo: { bar: 'baz' } }, escape), '')

  var { template } = await compile('<if foo is not present>baz</if>')
  assert.deepEqual(template({}, escape), 'baz')

  var { template } = await compile('<if foo is not present>bar</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'bar')

  var { template } = await compile('<if foo is defined>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')

  var { template } = await compile('<if foo is defined>{bar}</if>')
  ...
  var { template } = await compile('<if foo are present>bar</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'bar')

  var { template } = await compile('<if foo are present>bar</if>')
  assert.deepEqual(template({ foo: 'qux' }, escape), 'bar')

  var { template } = await compile('<if foo are not present>bar</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'bar')
  ...
  var { template } = await compile('<if foo is positive>baz</if>')
  ...
  var { template } = await compile('<if foo is_positive>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), 'baz')

  var { template } = await compile('<if foo is not positive>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), '')
  ...
  var { template } = await compile('<if foo is alpha>baz</if>')
  ...
  var { template } = await compile('<if foo is not alpha>baz</if>')
  assert.deepEqual(template({ foo: 'bar' }, escape), '')
  ...
  var { template } = await compile('<if foo is not alphanumeric>baz</if>')
  ...
  var { template } = await compile('<if foo is negative>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), '')
  ...
  var { template } = await compile('<if foo is not negative>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is finite>baz</if>')
  ...
  var { template } = await compile('<if foo is not finite>baz</if>')
  ...
  var { template } = await compile('<if foo is infinite>baz</if>')
  assert.deepEqual(template({ foo: 100 }, escape), '')
  ...
  var { template } = await compile('<if foo is not infinite>baz</if>')
  assert.deepEqual(template({ foo: Infinity }, escape), '')

  var { template } = await compile('<if foo is not infinite>baz</if>')
  assert.deepEqual(template({ foo: -Infinity }, escape), '')

  var { template } = await compile('<if foo is empty>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not empty>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), '')
  ...
  var { template } = await compile('<if foo are empty>baz</if>')
  ...
  var { template } = await compile('<if foo are not empty>baz</if>')
  ...
  var { template } = await compile('<if foo is an empty array>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'baz')

  var { template } = await compile('<if foo is an empty array>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4] }, escape), '')

  var { template } = await compile('<if foo is not an empty array>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4] }, escape), 'baz')

  var { template } = await compile('<if foo is an empty string>baz</if>')
  assert.deepEqual(template({ foo: '' }, escape), 'baz')

  var { template } = await compile('<if foo is an empty string>baz</if>')
  assert.deepEqual(template({ foo: 'foo' }, escape), '')

  var { template } = await compile('<if foo is not an empty string>baz</if>')
  assert.deepEqual(template({ foo: 'foo' }, escape), 'baz')

  var { template } = await compile('<if foo is an empty set>baz</if>')
  assert.deepEqual(template({ foo: new Set([]) }, escape), 'baz')

  var { template } = await compile('<if foo is an empty set>baz</if>')
  assert.deepEqual(template({ foo: new Set([1, 2, 3, 4]) }, escape), '')

  var { template } = await compile('<if foo is not an empty set>baz</if>')
  assert.deepEqual(template({ foo: new Set([1, 2, 3, 4]) }, escape), 'baz')

  var { template } = await compile('<if foo is an empty map>baz</if>')
  assert.deepEqual(template({ foo: new Map([]) }, escape), 'baz')

  var { template } = await compile('<if foo is an empty map>baz</if>')
  assert.deepEqual(template({ foo: new Map([ [{}, 'bar'] ]) }, escape), '')

  var { template } = await compile('<if foo is not an empty map>baz</if>')
  assert.deepEqual(template({ foo: new Map([ [{}, 'bar'] ]) }, escape), 'baz')

  var { template } = await compile('<if foo is an array>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not an array>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), '')
  ...
  var { template } = await compile('<if foo is a string>baz</if>')
  assert.deepEqual(template({ foo: '' }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not a string>baz</if>')
  assert.deepEqual(template({ foo: '' }, escape), '')
  ...
  var { template } = await compile('<if foo is a number>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), '')
  ...
  var { template } = await compile('<if foo is not a number>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is a multiple of bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not a multiple of bar>baz</if>')
  assert.deepEqual(template({ foo: 42, bar: 9 }, escape), 'baz')

  var { template } = await compile('<if foo is numeric>baz</if>')
  ...
  var { template } = await compile('<if foo is not numeric>baz</if>')
  ...
  var { template } = await compile('<if foo is a symbol>baz</if>')
  assert.deepEqual(template({ foo: Symbol('foo') }, escape), 'baz')

  var { template } = await compile('<if foo is a symbol>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is not a symbol>baz</if>')
  assert.deepEqual(template({ foo: Symbol('foo') }, escape), '')

  var { template } = await compile('<if foo is not a symbol>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')

  var { template } = await compile('<if foo is a map>baz</if>')
  assert.deepEqual(template({ foo: new Map() }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not a map>baz</if>')
  assert.deepEqual(template({ foo: new Map() }, escape), '')
  ...
  var { template } = await compile('<if foo is a weakmap>baz</if>')
  ...
  var { template } = await compile('<if foo is not a weakmap>baz</if>')
  ...
  var { template } = await compile('<if foo is a set>baz</if>')
  assert.deepEqual(template({ foo: new Set() }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not a set>baz</if>')
  assert.deepEqual(template({ foo: new Set() }, escape), '')
  ...
  var { template } = await compile('<if foo is a weakset>baz</if>')
  ...
  var { template } = await compile('<if foo is not a weakset>baz</if>')
  assert.deepEqual(template({ foo: new Set() }, escape), 'baz')

  var { template } = await compile('<if foo is a boolean>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), 'baz')

  var { template } = await compile('<if foo is a boolean>baz</if>')
  assert.deepEqual(template({ foo: false }, escape), 'baz')

  var { template } = await compile('<if foo is not a boolean>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), '')

  var { template } = await compile('<if foo is not a boolean>baz</if>')
  assert.deepEqual(template({ foo: '' }, escape), 'baz')

  var { template } = await compile('<if foo is undefined>baz</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'baz')

  var { template } = await compile('<if foo is undefined>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is not undefined>baz</if>')
  assert.deepEqual(template({ foo: undefined }, escape), '')

  var { template } = await compile('<if foo is not undefined>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')

  var { template } = await compile('<if foo is null>baz</if>')
  assert.deepEqual(template({ foo: null }, escape), 'baz')

  var { template } = await compile('<if foo is not null>baz</if>')
  assert.deepEqual(template({ foo: null }, escape), '')

  var { template } = await compile('<if foo is void>baz</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not void>baz</if>')
  assert.deepEqual(template({ foo: undefined }, escape), '')
  ...
  var { template } = await compile('<if foo is an object>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not an object>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')
  ...
  var { template } = await compile('<if foo is a regexp>baz</if>')
  ...
  var { template } = await compile('<if foo is not a regexp>baz</if>')
  ...
  var { template } = await compile('<if foo is a regex>baz</if>')
  assert.deepEqual(template({ foo: /regexp/ }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not a regex>baz</if>')
  assert.deepEqual(template({ foo: /regexp/ }, escape), '')
  ...
  var { template } = await compile('<if foo is a date>baz</if>')
  ...
  var { template } = await compile('<if foo is not a date>baz</if>')
  ...
  var { template } = await compile('<if foo is even>baz</if>')
  ...
  var { template } = await compile('<if foo is not even>baz</if>')
  ...
  var { template } = await compile('<if foo is odd>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not odd>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), '')
  ...
  var { template } = await compile('<if foo bitwise or bar>baz</if>')
  ...
  var { template } = await compile('<if foo bitwise and bar>baz</if>')
  assert.deepEqual(template({ foo: 0, bar: 0 }, escape), '')
  ...
  var { template } = await compile('<if foo bitwise xor bar>baz</if>')
  assert.deepEqual(template({ foo: 0, bar: 0 }, escape), '')
  ...
  var { template } = await compile('<if not foo>baz</if>')
  assert.deepEqual(template({ foo: null }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is a video>baz</if>')
  ...
  var { template } = await compile('<if foo is not a video>baz</if>')
  assert.deepEqual(template({ foo: 'fooflv' }, escape), 'baz')

  var { template } = await compile('<if foo is an image>baz</if>')
  ...
  var { template } = await compile('<if foo is not an image>baz</if>')
  assert.deepEqual(template({ foo: 'foo.bmp' }, escape), '')

  var { template } = await compile('<if foo is an audio>baz</if>')
  ...
  var { template } = await compile('<if foo is not an audio>baz</if>')
  assert.deepEqual(template({ foo: 'foomp3' }, escape), 'baz')

  var { template } = await compile('<if foo has an extension of bar>baz</if>')
  assert.deepEqual(template({ foo: 'foo.mp3', bar: 'mp3' }, escape), 'baz')

  var { template } = await compile('<if foo has extension of={"jpg"}>baz</if>')
  assert.deepEqual(template({ foo: 'foo.jpg' }, escape), 'baz')

  var { template } = await compile('<if foo has an extension of bar>baz</if>')
  assert.deepEqual(template({ foo: 'foo.mp3', bar: '.ogg' }, escape), '')

  var { template } = await compile('<if foo does not have an extension of bar>baz</if>')
  assert.deepEqual(template({ foo: 'foo.mp3', bar: '.ogg' }, escape), 'baz')

  var { template } = await compile('<if foo has a whitespace>baz</if>')
  ...
  var { template } = await compile('<if foo does not have a whitespace>baz</if>')
  assert.deepEqual(template({ foo: 'foobar' }, escape), 'baz')

  var { template } = await compile('<if foo does not have a whitespace>baz</if>')
  assert.deepEqual(template({ foo: 'foo bar' }, escape), '')

  var { template } = await compile('<if foo has a newline>baz</if>')
  assert.deepEqual(template({ foo: 'foo\nbar' }, escape), 'baz')

  var { template } = await compile('<if foo has a newline>baz</if>')
  assert.deepEqual(template({ foo: 'foo\tbar' }, escape), '')

  var { template } = await compile('<if foo does not have a newline>baz</if>')
  assert.deepEqual(template({ foo: 'foo\nbar' }, escape), '')

  var { template } = await compile('<if foo does not have a newline>baz</if>')
  assert.deepEqual(template({ foo: 'foo\tbar' }, escape), 'baz')

  var { template } = await compile('<if foo has a number>baz</if>')
  ...
  var { template } = await compile('<if foo is true>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not true>baz</if>')
  assert.deepEqual(template({ foo: false }, escape), 'baz')

  var { template } = await compile('<if foo is not true>baz</if>')
  assert.deepEqual(template({ foo: 'bar' }, escape), '')

  var { template } = await compile('<if foo is truthy>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not truthy>baz</if>')
  assert.deepEqual(template({ foo: false }, escape), 'baz')

  var { template } = await compile('<if foo is not truthy>baz</if>')
  assert.deepEqual(template({ foo: 'bar' }, escape), '')

  var { template } = await compile('<if foo is false>baz</if>')
  assert.deepEqual(template({ foo: false }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not false>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), 'baz')

  var { template } = await compile('<if foo is not false>baz</if>')
  assert.deepEqual(template({ foo: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo is falsy>baz</if>')
  assert.deepEqual(template({ foo: false }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not falsy>baz</if>')
  assert.deepEqual(template({ foo: true }, escape), 'baz')

  var { template } = await compile('<if foo is not falsy>baz</if>')
  assert.deepEqual(template({ foo: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo is divisible by bar>baz</if>')
  assert.deepEqual(template({ foo: 10, bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is not divisible by bar>baz</if>')
  assert.deepEqual(template({ foo: 5, bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is divisible by="{10}">baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is not divisible by="{10}">baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is divisible by five>baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is divisible by five>baz</if>')
  assert.deepEqual(template({ foo: 6 }, escape), '')

  var { template } = await compile('<if foo is not divisible by five>baz</if>')
  assert.deepEqual(template({ foo: 6 }, escape), 'baz')

  var { template } = await compile('<if foo is prime>baz</if>')
  ...
  var { template } = await compile('<if foo is not prime>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), 'baz')

  var { template } = await compile('<if foo is a palindrome>baz</if>')
  assert.deepEqual(template({ foo: 'madam' }, escape), 'baz')

  var { template } = await compile('<if foo is a palindrome>baz</if>')
  assert.deepEqual(template({ foo: 'foo' }, escape), '')

  var { template } = await compile('<if foo is not a palindrome>baz</if>')
  assert.deepEqual(template({ foo: 'foo' }, escape), 'baz')

  var { template } = await compile('<if foo is sooner than bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not sooner than bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 4, 29), bar: new Date(2018, 4, 11) }, escape), 'baz')

  var { template } = await compile('<if foo is later than bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 4, 29), bar: new Date(2018, 4, 11) }, escape), 'baz')

  var { template } = await compile('<if foo is later than bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 4, 1), bar: new Date(2018, 4, 29) }, escape), '')

  var { template } = await compile('<if foo is not later than bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 4, 1), bar: new Date(2018, 4, 29) }, escape), 'baz')

  var { template } = await compile('<if foo is before bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not before bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 5, 29), bar: new Date(2018, 5, 27) }, escape), 'baz')

  var { template } = await compile('<if foo is after bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 5, 29), bar: new Date(2018, 5, 27) }, escape), 'baz')

  var { template } = await compile('<if foo is after bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 5, 27), bar: new Date(2018, 5, 29) }, escape), '')

  var { template } = await compile('<if foo is not after bar>baz</if>')
  assert.deepEqual(template({ foo: new Date(2018, 5, 27), bar: new Date(2018, 5, 29) }, escape), 'baz')

  var { template } = await compile('<if foo is a digit>baz</if>')
  assert.deepEqual(template({ foo: 0 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is not a digit>baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is decimal>baz</if>')
  ...
  var { template } = await compile('<if foo is not decimal>baz</if>')
  assert.deepEqual(template({ foo: 1 }, escape), 'baz')

  var { template } = await compile('<if foo is frozen>baz</if>')
  assert.deepEqual(template({ foo: Object.freeze({}) }, escape), 'baz')

  var { template } = await compile('<if foo is frozen>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is not frozen>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')

  var { template } = await compile('<if foo is sealed>baz</if>')
  assert.deepEqual(template({ foo: Object.seal({}) }, escape), 'baz')

  var { template } = await compile('<if foo is sealed>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is not sealed>baz</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'baz')
  ...
  var { template } = await compile('<if foo eq="{10}">baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo eq="{100 + 100}">baz</if>')
  assert.deepEqual(template({ foo: 200 }, escape), 'baz')

  var { template } = await compile('<if foo eq="{100 + 100 + 0}">baz</if>')
  assert.deepEqual(template({ foo: 200 }, escape), 'baz')
  ...
  var { template } = await compile('<if foo starts with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'ban' }, escape), 'baz')

  var { template } = await compile('<if foo starts with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'qux' }, escape), '')

  var { template } = await compile('<if foo does not start with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'qux' }, escape), 'baz')

  var { template } = await compile('<if foo ends with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'qux' }, escape), 'baz')

  var { template } = await compile('<if foo ends with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'ban' }, escape), '')

  var { template } = await compile('<if foo does not end with bar>baz</if>')
  assert.deepEqual(template({ foo: 'ban qux', bar: 'ban' }, escape), 'baz')

  var { template } = await compile('<if foo and bar and baz>qux</if>')
  ...
  var { template } = await compile('<if foo and bar and baz and ban>qux</if>')
  ...
  var { template } = await compile('<if foo and bar equals baz>qux</if>')
  ...
  var { template } = await compile('<if foo is divisible by three and foo is divisible by five>bar</if>')
  assert.deepEqual(template({ foo: 15 }, escape), 'bar')

  var { template } = await compile('<if foo is divisible by three and foo is divisible by five>bar</if>')
  assert.deepEqual(template({ foo: 14 }, escape), '')

  var { template } = await compile('<if foo is divisible by three and foo is divisible by four>bar</if>')
  assert.deepEqual(template({ foo: 12 }, escape), 'bar')

  var { template } = await compile('<if foo is divisible by two and foo is divisible by five>bar</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'bar')

  var { template } = await compile('<if foo is divisible by three and foo is divisible by five>bar</if>')
  assert.deepEqual(template({ foo: 8 }, escape), '')

  var { template } = await compile('<if foo includes bar>baz</if>')
  ...
  var { template } = await compile('<if foo does not include={"dolor"}>baz</if>')
  assert.deepEqual(template({ foo: ['lorem', 'ipsum'] }, escape), 'baz')

  var { template } = await compile('<if foo contains={"ipsum"}>baz</if>')
  assert.deepEqual(template({ foo: ['lorem', 'ipsum'] }, escape), 'baz')

  var { template } = await compile('<if foo contains={"dolor"}>baz</if>')
  assert.deepEqual(template({ foo: ['lorem', 'ipsum'] }, escape), '')

  var { template } = await compile('<if foo does not contain={"dolor"}>baz</if>')
  assert.deepEqual(template({ foo: ['lorem', 'ipsum'] }, escape), 'baz')
  ...
  var { template } = await compile('<if foo matches bar>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: /ipsum/ }, escape), 'baz')

  var { template } = await compile('<if foo matches bar>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: /dolor/ }, escape), '')

  var { template } = await compile('<if foo does not match bar>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: /dolor/ }, escape), 'baz')

  var { template } = await compile('<if not foo and bar>baz</if>')
  assert.deepEqual(template({ foo: false, bar: true }, escape), 'baz')
  ...
  var { template } = await compile('<if foo is positive and bar>baz</if>')
  ...
  var { template } = await compile('<if foo is not positive and bar>baz</if>')
  assert.deepEqual(template({ foo: 10, bar: true }, escape), '')
  ...
  var { template } = await compile('<if foo is not positive and not bar>baz</if>')
  assert.deepEqual(template({ foo: 10, bar: true }, escape), '')
  ...
  var { template } = await compile('<if foo and not bar>baz</if>')
  assert.deepEqual(template({ foo: false, bar: true }, escape), '')
  ...
  var { template } = await compile('<if foo responds to={"bar"}>baz</if>')
  assert.deepEqual(template({ foo: { bar () {} } }, escape), 'baz')

  var { template } = await compile('<if foo responds bar>baz</if>')
  assert.deepEqual(template({ foo: { bar () {} }, bar: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo responds bar>baz</if>')
  assert.deepEqual(template({ foo: { bar: [] }, bar: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo does not respond to bar>baz</if>')
  assert.deepEqual(template({ foo: { bar: [] }, bar: 'bar' }, escape), 'baz')

  var { template } = await compile('<if foo bitwise and bar or baz>baz</if>')
  assert.deepEqual(template({ foo: 1, bar: 1, baz: 0 }, escape), 'baz')

  var { template } = await compile('<if foo bitwise and bar or baz>baz</if>')
  assert.deepEqual(template({ foo: 0, bar: 0, baz: 0 }, escape), '')

  var { template } = await compile('<if foo is an email>baz</if>')
  assert.deepEqual(template({ foo: 'as@ts.eu' }, escape), 'baz')

  var { template } = await compile('<if foo is an email>baz</if>')
  assert.deepEqual(template({ foo: 'asts.eu' }, escape), '')

  var { template } = await compile('<if foo have more than one element>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if foo have more than four item>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if photos have more than two photo>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), '')

  var { template } = await compile('<if photos do not have more than two photo>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if foo have less than six element>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if photos have less than two photo>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), '')

  var { template } = await compile('<if photos do not have less than two photo>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if photos have two photo>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if photos have zero photo>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), 'baz')

  var { template } = await compile('<if photos have one photo>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), '')

  var { template } = await compile('<if photos do not have one photo>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), 'baz')

  var { template } = await compile('<if photos have many pictures>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')
  ...
  var { template } = await compile('<if photos do not have many pictures>baz</if>')
  assert.deepEqual(template({ photos: [{}] }, escape), 'baz')

  var { template } = await compile('<if photos have elements>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')
  ...
  var { template } = await compile('<if photos do not have elements>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), 'baz')

  var { template } = await compile('<if foo has more than one element>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if foo has more than four item>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if foo has more than four item>baz</if>')
  assert.deepEqual(template({ foo: [{}, {}] }, escape), '')

  var { template } = await compile('<if photos has more than two picture>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), '')

  var { template } = await compile('<if photos does not have more than two picture>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if foo has less than six element>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5] }, escape), 'baz')

  var { template } = await compile('<if photos has less than two item>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), '')

  var { template } = await compile('<if photos does not have less than two item>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if photos has two element>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if photos has zero item>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), 'baz')

  var { template } = await compile('<if photos has one picture>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), '')

  var { template } = await compile('<if photos does not have one element>baz</if>')
  assert.deepEqual(template({ photos: [] }, escape), 'baz')

  var { template } = await compile('<if photos has many pictures>baz</if>')
  assert.deepEqual(template({ photos: [{}, {}] }, escape), 'baz')

  var { template } = await compile('<if photos has many pictures>baz</if>')
  assert.deepEqual(template({ photos: [{}] }, escape), '')

  var { template } = await compile('<if photos does not have many pictures>baz</if>')
  assert.deepEqual(template({ photos: [{}] }, escape), 'baz')

  var { template } = await compile('<if foo has items>baz</if>')
  assert.deepEqual(template({ foo: [{}, {}] }, escape), 'baz')
  ...
  var { template } = await compile('<if foo does not have items>baz</if>')
  assert.deepEqual(template({ foo: [] }, escape), 'baz')

  var { template } = await compile('<if foo is between bar and baz>baz</if>')
  ...
  var { template } = await compile('<if foo is between one and ten>baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is between six and nine>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), '')

  var { template } = await compile('<if foo is not between six and nine>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is between bar and baz and qux>baz</if>')
  ...
  var { template } = await compile('<if foo is not between bar and baz and qux>baz</if>')
  assert.deepEqual(template({ foo: 0, bar: 10, baz: 20, qux: true }, escape), 'baz')

  var { template } = await compile('<if foo is below bar>baz</if>')
  assert.deepEqual(template({ foo: 5, bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is below bar>baz</if>')
  assert.deepEqual(template({ foo: 20, bar: 20 }, escape), '')

  var { template } = await compile('<if foo is below six>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is below three>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), '')

  var { template } = await compile('<if foo is not below three>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is not below three>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is above bar>baz</if>')
  assert.deepEqual(template({ foo: 15, bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo is above bar>baz</if>')
  assert.deepEqual(template({ foo: 20, bar: 20 }, escape), '')

  var { template } = await compile('<if foo is above six>baz</if>')
  assert.deepEqual(template({ foo: 15 }, escape), 'baz')

  var { template } = await compile('<if foo is above nine>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), '')

  var { template } = await compile('<if foo is not above nine>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is not above three>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), '')

  var { template } = await compile('<if foo is at least bar>baz</if>')
  assert.deepEqual(template({ foo: 20, bar: 20 }, escape), 'baz')

  var { template } = await compile('<if foo is at least four>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is at least bar>baz</if>')
  assert.deepEqual(template({ foo: 10, bar: 20 }, escape), '')

  var { template } = await compile('<if foo is at least eight>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), '')

  var { template } = await compile('<if foo is not at least bar>baz</if>')
  assert.deepEqual(template({ foo: 10, bar: 20 }, escape), 'baz')

  var { template } = await compile('<if foo is not at least eight>baz</if>')
  assert.deepEqual(template({ foo: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is at most bar>baz</if>')
  assert.deepEqual(template({ foo: 20, bar: 20 }, escape), 'baz')

  var { template } = await compile('<if foo is at most bar>baz</if>')
  assert.deepEqual(template({ foo: 30, bar: 20 }, escape), '')

  var { template } = await compile('<if foo is not at most bar>baz</if>')
  assert.deepEqual(template({ foo: 30, bar: 20 }, escape), 'baz')

  var { template } = await compile('<if foo is at most eight>baz</if>')
  assert.deepEqual(template({ foo: 6 }, escape), 'baz')

  var { template } = await compile('<if foo is at most eight>baz</if>')
  assert.deepEqual(template({ foo: 20 }, escape), '')

  var { template } = await compile('<if foo is not at most eight>baz</if>')
  assert.deepEqual(template({ foo: 10 }, escape), 'baz')

  var { template } = await compile('<if foo has length of bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo has length of bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), '')

  var { template } = await compile('<if foo does not have length of bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo has length of five>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo has length of six>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), '')

  var { template } = await compile('<if foo has length of at least bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo has length of at least bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), '')

  var { template } = await compile('<if foo does not have length of at least bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 10 }, escape), 'baz')

  var { template } = await compile('<if foo has length of at least five>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo has length of at least bar>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: 100 }, escape), '')

  var { template } = await compile('<if foo has length of at most bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo has length of at most bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 1 }, escape), '')

  var { template } = await compile('<if foo does not have length of at most bar>baz</if>')
  assert.deepEqual(template({ foo: [1, 2, 3, 4, 5], bar: 1 }, escape), 'baz')

  var { template } = await compile('<if foo has length of at most five>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: 5 }, escape), '')

  var { template } = await compile('<if foo does not have length of at most bar>baz</if>')
  assert.deepEqual(template({ foo: 'lorem ipsum', bar: 5 }, escape), 'baz')

  var { template } = await compile('<if foo is different than bar>baz</if>')
  ...
  var { template } = await compile('<if foo is in bar>baz</if>')
  ...
  var { template } = await compile('<if foo is a url><a href="{foo}">{bar}</a></if>')
  ...
  var { template } = await compile('<if foo>bar</if><else>baz<if qux>quux</if></else>')
  ...
  var { template } = await compile('<if foo>bar</if><elseif bar>baz<if qux>quux</if></elseif>')
  ...
  var { template } = await compile('<if foo>foo</if><elseif bar>bar</elseif><else><if baz>baz</if><else>ban</else></else>')
  ...
  var { template } = await compile('<if foo>bar</if><elseunless bar>baz</elseunless>')
  assert.deepEqual(template({ foo: false, bar: false }, escape), 'baz')
  ...
  var { template } = await compile('<if foo>bar</if><elseif bar>baz</else>')
  assert.deepEqual(template({ foo: true, bar: false }, escape), 'bar')

  var { template } = await compile('<if foo>bar</if><elseif bar>baz</else>')
  assert.deepEqual(template({ foo: false, bar: true }, escape), 'baz')

  var { template } = await compile('<if foo is extensible>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'bar')

  var { template } = await compile('<if foo is not extensible>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is not extensible>bar</if>')
  assert.deepEqual(template({ foo: Object.seal({}) }, escape), 'bar')

  var { template } = await compile('<if foo is an error>bar</if>')
  assert.deepEqual(template({ foo: new Error('ValidationError') }, escape), 'bar')

  var { template } = await compile('<if foo is not an error>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'bar')

  var { template } = await compile('<if foo has any keys>bar</if>')
  ...
  var { template } = await compile('<if foo does not have any keys>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'bar')

  var { template } = await compile('<if foo is nan>bar</if>')
  assert.deepEqual(template({ foo: NaN }, escape), 'bar')

  var { template } = await compile('<if foo is not nan>bar</if>')
  assert.deepEqual(template({ foo: null }, escape), 'bar')

  var { template } = await compile('<if foo exists>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), 'bar')

  var { template } = await compile('<if foo exists>bar</if>')
  assert.deepEqual(template({ foo: undefined }, escape), '')

  var { template } = await compile('<if foo not exist>bar</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'bar')

  var { template } = await compile('<if foo is missing>bar</if>')
  assert.deepEqual(template({ foo: {} }, escape), '')

  var { template } = await compile('<if foo is missing>bar</if>')
  assert.deepEqual(template({ foo: undefined }, escape), 'bar')

  var { template } = await compile('<if foo is not missing>bar</if>')
  assert.deepEqual(template({ foo: null }, escape), 'bar')

  var { template } = await compile('<if foo is function>bar</if>')
  assert.deepEqual(template({ foo: () => {} }, escape), 'bar')
  ...
  var { template } = await compile('<if foo has type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'object' }, escape), 'bar')

  var { template } = await compile('<if foo has type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'string' }, escape), '')

  var { template } = await compile('<if foo does not have type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'string' }, escape), 'bar')

  var { template } = await compile('<if foo ist type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'object' }, escape), 'bar')

  var { template } = await compile('<if foo is type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'string' }, escape), '')

  var { template } = await compile('<if foo is not type of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: 'string' }, escape), 'bar')

  var { template } = await compile('<if foo is an instance of bar>bar</if>')
  assert.deepEqual(template({ foo: new Date(), bar: Date }, escape), 'bar')

  var { template } = await compile('<if foo is an instance of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: Date }, escape), '')

  var { template } = await compile('<if foo is not an instance of bar>bar</if>')
  assert.deepEqual(template({ foo: {}, bar: Date }, escape), 'bar')
})

test('if: words to numbers', async assert => {
  var { template } = await compile('<if number equals zero>foo</if><else>bar</else>')
  assert.deepEqual(template({ number: 0 }, escape), 'foo')
  assert.deepEqual(template({ number: 1 }, escape), 'bar')

  var { template } = await compile('<if number equals eleven>foo</if><else>bar</else>')
  assert.deepEqual(template({ number: 11 }, escape), 'foo')
  assert.deepEqual(template({ number: 12 }, escape), 'bar')

  var { template } = await compile('<if number equals one-hundred>foo</if><else>bar</else>')
  assert.deepEqual(template({ number: 100 }, escape), 'foo')
  assert.deepEqual(template({ number: 101 }, escape), 'bar')

  var { template } = await compile('<if number equals four-thousand>foo</if><else>bar</else>')
...
test('if: numbers', async assert => {
  var { template } = await compile('<if photos.length equals 0>foo</if>')
...
test('if: variables with curly brackets', async assert => {
  ...
  var { template } = await compile('<if {2+2} equals 4>foo</if>')
