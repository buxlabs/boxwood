...
test.skip('attributes: shorthand syntax with one item', async assert => {
...
test.skip('attributes: shorthand syntax with two items', async assert => {
...
test.skip('attributes: shorthand syntax with and operator', async assert => {
  var { template } = await compile(`<div class="[foo && 'bar']"></div>`)
  assert.deepEqual(template({}, escape), '<div class=""></div>')
...
test.skip('attributes: shorthand syntax with or operator', async assert => {
  var { template } = await compile(`<div class="[foo || 'bar']"></div>`)
  assert.deepEqual(template({}, escape), '<div class="bar"></div>')
  assert.deepEqual(template({ foo: 'foo' }, escape), '<div class="foo"></div>')
})

test.skip('attributes: shorthand syntax for multiple or operators', async assert => {
...
test.skip('attributes: shorthand syntax with ternary operator', async assert => {
...
test.skip('attributes: shorthand syntax with one string', async assert => {
  var { template } = await compile(`<div class="['foo', bar]"></div>`)
  assert.deepEqual(template({}, escape), '<div class="foo"></div>')
  assert.deepEqual(template({ bar: 'bar' }, escape), '<div class="foo bar"></div>')
})

test.skip('attributes: shorthand syntax with multiple strings', async assert => {
...
test.skip('attributes: shorthand syntax for passing data to the nested components with template data', async assert => {
  var { template } = await compile(`
    <import layout from="./layout.html"/>
    <layout {picture}/>
  `, { paths: [join(__dirname, '../../fixtures/attributes/shorthand')] })
