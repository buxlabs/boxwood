...
test('import', async assert => {
  ...
  var { template } = await compile(`<render partial="./header.html" {title}>`, {
    paths: [ path.join(__dirname, '../../fixtures/partial') ]
  })
...
test('import: should be possible to pass functions', async assert => {
  var { template } = await compile(`<import foo from="./foo.html"><foo foo="{foo}"/>`, {
...
test('import: should be possible to pass anonymous functions', async assert => {
  var { template } = await compile(`<import foo from="./foo.html"><foo foo="{foo}"/>`, {
    paths: [ path.join(__dirname, '../../fixtures/import/functions') ]
  })
  assert.deepEqual(template({ foo: function () { return 'bar' } }, escape), '<div>bar</div>')
})
...
test('import: shorthand conditions syntax in the imported component', async assert => {
...
test('import: partial should be optimized safely', async assert => {
  var { template } = await compile(`<render partial="./header.html" {title}></render>`, {
    paths: [ path.join(__dirname, '../../fixtures/partial') ]
  })
  assert.deepEqual(template({ title: 'foo' }, escape), '<div>foo</div>')
})
