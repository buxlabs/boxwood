...
test('div[margin]: works with values', async assert => {
  var { template } = await compile(`<div margin="15px"></div>`)
  assert.deepEqual(template({}, escape), '<div style="margin: 15px;"></div>')
})

test('div[margin]: works with expressions', async assert => {
  var { template } = await compile(`<div id="qux" class="foo bar ban" margin="{15}"></div>`)
...
test('div[margin]: works with object', async assert => {
  var { template } = await compile(`<div margin="{{ top: 15 }}"></div>`)
...
test('div[margin]: works with predefined style variables', async assert => {
  ...
  var { template } = await compile(`<div marginTop="small"></div>`, { style })
  assert.deepEqual(template({}, escape), '<div style="margin-top: 10px;"></div>')

  var { template } = await compile(`<div margin="{{ top: 'small' }}"></div>`, { style })
  assert.deepEqual(template({}, escape), '<div style="margin-top: 10px;"></div>')

  var { template } = await compile(`<div margin="{{ bottom: 'small', top: 'big' }}"></div>`, { style })
  assert.deepEqual(template({}, escape), '<div style="margin-bottom: 10px; margin-top: 40px;"></div>')

  var { template } = await compile(`<div margin-bottom='big' margin-top='small'></div>`, { style })
